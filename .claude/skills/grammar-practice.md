...
```
正確：その {問題|もんだい}について、{部長|ぶちょう}と {話し合う|はなしあう}
錯誤：その問題について、部長と話し合う
```
...
**問題：** AI 在同一個 context 中知道答案，容易不小心洩漏。
...
2. **打亂選項順序**：
   - 使用 `(題號 + 日期數字) % 4` 決定正解位置
   - 例：第 1 題 + 20260128 → (1 + 20260128) % 4 = 1 → 正解在選項 2
   - 例：第 2 題 + 20260128 → (2 + 20260128) % 4 = 2 → 正解在選項 3

3. **輸出時**：
   - 只輸出打亂後的選項，不要在任何地方記錄或提及正解位置
   - 不要輸出 answer key
   - 不要在範例中給出答案格式（如「2 3 2 3」）

4. **批改時**：
   - 重新計算 `(題號 + 日期數字) % 4` 來還原正解位置
   - 此時才顯示正解與解析

**嚴禁事項：**
- ❌ 出題時在選項旁標示正解
- ❌ 在提示中給出可能答案的範例
- ❌ 在檔案中寫入 answer key 或 metadata
- ❌ 在心中默念正解（會影響輸出）
...
- 解析要說明「為什麼對」和「為什麼錯」
- 最後要有重點整理表格
- **全題解析**：不只是錯題，正確的題目也要有翻譯和解析
- **答案標記格式**：正解句中的填空答案使用 `<b><u>答案</u></b>` 標記（粗體+下劃線）
